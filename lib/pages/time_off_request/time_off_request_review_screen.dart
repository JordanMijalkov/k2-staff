...
class TimeOffRequestReviewScreen extends StatefulWidget {
  const TimeOffRequestReviewScreen({Key? key}) : super(key: key);

  @override
  State<TimeOffRequestReviewScreen> createState() =>
      _TimeOffRequestReviewScreenState();
}

class _TimeOffRequestReviewScreenState
    extends State<TimeOffRequestReviewScreen> {
  Future<List<K2TimeOffRequest>> _getTimeOffRequests() async {
    var list = await context.read<TimeOffRequestCubit>().getMyTimeOffRequests(
        DateTime.now().toIso8601String(),
        DateTime(2022, 12, 31).toIso8601String());
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Current Requests', style: Theme.of(context).textTheme.headline3),
                Padding(
...
//                      width: width,
                      child: ElevatedButton(
                        onPressed: () {
                          context.read<TimeOffRequestCubit>().timeOffHistory();
                        },
                        child: Text(
                          'Time Off History', style: TextStyle(fontSize: 12),
                        ),
                      )),
                  SizedBox(width: 12,),
                  SizedBox(
